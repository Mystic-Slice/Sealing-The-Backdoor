...
def get_attn_maps(tokenizer, prompt, timestep=None, max_height=256, max_width=256):
    resized_map = None
    ...
    for i, (token, token_attn_map) in enumerate(zip(tokens, resized_map)):
        ...
        # min-max normalization(for visualization purpose)
        token_attn_map = token_attn_map.detach().numpy()
        normalized_token_attn_map = (token_attn_map - np.min(token_attn_map)) / (np.max(token_attn_map) - np.min(token_attn_map)) # * 255
        # normalized_token_attn_map = normalized_token_attn_map.astype(np.uint8)

        attn_maps_final.append((token, torch.from_numpy(normalized_token_attn_map)))

    return attn_maps_final
...
def get_maps_one_timestep(tokenizer, prompt, max_height, max_width, save_path='attn_maps_by_timesteps'):
    for timestep in attn_maps.keys():
        return get_attn_maps(tokenizer, prompt, timestep, max_height=max_height, max_width=max_width)